(function($, window, document, undefined){
  ...
  sliderProto.getStarted = function(){
    ...
    // check browser support for 3d transforms and touch events
    this.checkSupport();
  ...
  sliderProto.goPrevious = function(){
    if (this.currItem < this.opts.numItems) {
      ...
      this.currItem -= this.opts.slideAsGroup ? this.opts.numItems : (this.opts.itemsToSlide || 1);
    }
  ...
  sliderProto.goNext = function(){
    var itemsLeft = this.contentCount - this.opts.numItems - this.currItem;

    if ((this.contentCount % this.opts.numItems) && (itemsLeft < this.opts.numItems)) {
      this.currItem += itemsLeft;

    } else if (this.currItem < (this.contentCount - this.opts.numItems)) {
      this.currItem += this.opts.slideAsGroup ? this.opts.numItems : (this.opts.itemsToSlide || 1);
    }
  ...
  sliderProto.goToSlide = function(slide, shortSlide){
    var position = slide * this.itemWidth * -1,
    ...
    // Add no click if at the end
    if (position * -1 === this.itemWidth * (this.contentCount - this.opts.numItems)) {
