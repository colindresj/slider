;(function($, window, document, Reajs, undefined){
  ...
  TNY.Slider = function(element, options){
    this.defaults = {
      jsonUrl: 'https://dl.dropboxusercontent.com/u/27069771/seeds.json',
      beforeInit: $.noop(),
      beforeLoad: $.noop(),
      beforeSlide: $.noop(),
      afterSlide: $.noop(),
    ...
    // Expose an API for interacting with the slider programmatically
    return {
      stop: function(){
        this.stopAutoPlay();
      }.bind(this),
    ...
    xhrSuccess = function(data){
      ...
      this.imagePromise = $.Deferred()
                           .done(function(){
                            ...
                            // Slow brower rendering hack
                            // Reques the dispaly to the end of the execution queue
                            // So the rendering threads can catch up with the script
                             setTimeout(function(){
  ...
  sliderProto.startSlider = function(){
    ...
    if (this.opts.auto.play) {
      this.autoPlayer = setInterval(this.autoPlay.bind(this), this.opts.auto.delay);
    }
  };
  ...
  sliderProto.createNavigation = function(){
    ...
    arrowEvents = function(e){
      ...
      if (this.opts.auto.play) this.stopAutoPlay();
    ...
    if (this.opts.navigation.arrows) {
    ...
    if (this.opts.navigation.pagination) {
  ...
  sliderProto.createSwipeEventListener = function(){
    var swipeSlider, startTime, endTime, timeMin = 100,
        distanceMin = 100, distance = 0, swipeDirection,
        initTouchPoint, initX, exitTouchPoint;

    // Return out of not a touch device
    if (!this.compatability.touch) return;

    swipeSlider = function(e) {
    ...
    this.el.addEventListener('touchmove', function(e){
      e.preventDefault(); // Prevent scrolling while swiping inside the slider
    });
    ...
    this.el.addEventListener('touchend', function(e){
      ...
      if (endTime - startTime > timeMin) {
        this.triggerCustomEvent('tnySlider:swipe', { direction: swipeDirection });
      }
    }.bind(this));

    this.el.addEventListener('tnySlider:swipe', swipeSlider.bind(this));
  };
  ...
  sliderProto.lazyLoad = function(){
    ...
    // Return out if not set to lazy load
    if (!this.opts.lazy.load) return;

    // Return if all the images have been loaded
  ...
  sliderProto.loadImage = function($item, $lazyImage){
    $lazyImage[0].src = $lazyImage.data('load-src');
    // $lazyImage.attr('src', $lazyImage.data('load-src'));
    $item.data('load-status', 'loaded');
    $lazyImage.removeAttr('data-load-src').attr('class', 'lazyLoaded');

    return this.opts.lazy.fadeIn ? $lazyImage.fadeIn(800) : $lazyImage[0].style.display = 'inline';
  };

  sliderProto.resolve = function(){
    var i, tryAgain;
    ...
    for (i = 0; i < this.opts.numItems; i++) {

      // If not loaded, wait a bit and call again
      if (!isImageOk(this.$contentItems[i])) {
        setTimeout(tryAgain.bind(this), 100);
      }
    }

    this.imagePromise.resolve.call(this);

  };

  sliderProto.createAd = function(){
    var adEl = document.createElement('div');
    adEl.className = this.opts.ads.className;

    return adEl;
  };

  sliderProto.insertAds = function(){
    var adEl, i, slides, _slides = document.getElementsByClassName('slide');

    function injectAd(adEl, beforeItem){
       beforeItem.parentNode.insertBefore(adEl, beforeItem.nextSibling);
    }

    slides = [].slice.call(_slides, 0);

    for(i = this.opts.ads.interval; i < this.contentCount; i += this.opts.ads.interval) {
      adEl = this.createAd();
      injectAd(adEl, slides[i]);
      slides.splice(i, 0, adEl);
      i++;
    }
  };
